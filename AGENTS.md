...
- **Assigned review-workflow badges** (`git/GitHubApi.js` + `WiViewRender.actions2html`): extra search queries decorate PRs in the Assigned view with `custom_actions` badges — `review_request` (I'm a requested reviewer), `changes_requested` (I authored a PR with changes requested), `pending_merge` (approved-but-open PR I authored or reviewed; opt-out per provider via `enablePendingMerge`). Because the search API can't tell "whose turn it is", an async GraphQL refinement after paint (`WiController.dispatchReviewStates` → `GitHubApi.updateReviewStatesAsync` → `GitHubAdapter.reviewRequests2decisions` → `WiView.muteChangesRequestedBadge`) mutes the `changes_requested` badge to "in review" once the author re-requested review (`reviewRequests` not empty). **GitLab mirrors this on the opposite side of the workflow (reviewer role):** `dispatchReviewStates` collects MRs carrying `review_request` and the async refinement (`GitLabApi.updateReviewStatesAsync` → `GitLabGraphql.getReviewStatesQuery` → `GitLabAdapter.reviewStates2decisions` → `WiController.updateReviewRequestStates` → `WiView.muteReviewRequestBadge`) mutes the `review_request` badge to "changes requested" once my own `reviewers.nodes[].mergeRequestInteraction.reviewState` is `REQUESTED_CHANGES` (ball back with the author; GitLab resets it when the author re-requests review). **Still GitHub-only: `pending_merge` — the GitLab mirror of that one badge is pending.**
