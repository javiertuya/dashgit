...
const wiControllerFollowUp = {
  ...
  edit: async function (params) {
    ...
    let followUps = response.content;
  ...
  save: async function (params, isDelete) {
    const fileName = this.getFileName(params.server)
    ...
    await this.update(fileName, sha, followUps, message, "Done: " + message);
  },
  ...
  getFileName: function (server) {
    return config.param.followUpFolder + "/" + server.replace("https://", "").replaceAll("/", "_") + ".json";
  },

  // Gets the json file with the follow ups of a provider, 
  // if not found creates a dedicated branch and a default file with an empty array
  read: async function (params) {
    ...
    try {
      ...
      const response = await gitHubApi.getContent(config.data.updateManagerToken, ownerRepo[0], ownerRepo[1], config.param.followUpBranch, fileName);
      ...
      if (error.status == 404) { // not found, branch+file or file only are missing
        ...
        try {
          ...
          await gitHubApi.createBranch(config.data.updateManagerToken, ownerRepo[0], ownerRepo[1], config.param.followUpBranch);
        } catch (error2) {
        ...
        await this.update(fileName, "", [], `Created follow up storage for ${params.server}`, "");
        return { sha: "", content: [] };
      } else {
        this.displayError(error);
        throw error;
  ...
  update: async function (fileName, sha, content, commitMessage, uiMessage) {
    ...
    // Approach:
    // - Set a delay before updating, the frequency of the error is much smaller (I've reproduced only twice in tests)
    // - Capture the exception and warn the user with a message in the modal window
    try {
      await new Promise(r => setTimeout(r, 300));
      let response = await gitHubApi.updateContent(config.data.updateManagerToken, ownerRepo[0], ownerRepo[1],
